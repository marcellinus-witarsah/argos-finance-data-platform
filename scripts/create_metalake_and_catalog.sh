...
if echo "$response" | grep -q "\"code\":0"; then
  ...
  response=$(curl -X POST -H "Accept: application/vnd.gravitino.v1+json" -H "Content-Type: application/json" -d '{ "name":"'$GRAVITINO_CATALOG_NAME'", "type":"RELATIONAL", "provider":"lakehouse-iceberg", "comment":"comment", "properties":{ "uri":"jdbc:postgresql://postgres:5432/catalog_metastore_db", "catalog-backend":"jdbc", "warehouse":"s3a://iceberg/warehouse", "jdbc-user":"postgres", "jdbc-password":"postgres", "jdbc-driver":"org.postgresql.Driver"} }' http://gravitino:8090/api/metalakes/$GRAVITINO_METALAKE_NAME/catalogs)
  if echo "$response" | grep -q "\"code\":0"; then
    true # Placeholder, do nothing
  else
