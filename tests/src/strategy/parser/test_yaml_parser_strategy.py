...
def __make_yaml_parser_strategy():
    return YAMLParserStrategy()

def test_if_yaml_filepath_invalid_return_none():
    mock_invalid_filepath = "./mock_invalid_filepath.yaml"

    yaml_parser_strategy = __make_yaml_parser_strategy()
    result = yaml_parser_strategy.parse(mock_invalid_filepath)

    assert result == None

def test_if_yaml_filepath_and_content_valid_return_a_dictionary():
    config_data = {
